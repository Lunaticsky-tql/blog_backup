...
前面两点原则容易理解，关于最后一点，因为我们编程其实更重要的是一个团队合作的一个过程，团队的整体工作效率也是非常重要的。比如字节内部采用Go语言，其实最有特点的一点就是Go语言的简洁性对于降低这个新成员上手项目代码成本很有帮助。在实际的这个工程项目当中，如果有一些复杂的一些程序逻辑，其他人基本上就不太敢动，尤其是有一些历史的一些逻辑的话，写得比较复杂，可能又难以理解，新接手的人就没办法明确的知道我们这些调整会造成的这个影响的一些范围，可能会产生什么样的问题。这样的话就会难以让这个代码难以维护。而且我们这些难以维护的这些逻辑在排查问题的时候也会带来不少麻烦。而如果我们的代码比较清晰的话，那么就算出现了一些问题，然后我们也能通过现象，或者代码逻辑，快速分析排查定位到这个问题。能够提升整体的整个项目开发环节的效率。所以说这也是非常重要的一点。
...
上述内容仅是个人结合实践和Effective Go总结出的自己认为较重要的部分记录，以便回顾和提醒。深入掌握一门语言还是要靠阅读第一手材料和动手实践。
